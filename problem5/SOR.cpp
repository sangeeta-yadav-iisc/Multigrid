...
x[row_index] = omega*(-csr_mv(A,x,row_index) + b[row_index])*pow(h,2.0)/2 + (1-omega)*x[row_index];}

cblas_daxpy(A->sizeofmatrix,-1.0,x,1,y,1);
rel_error=norm(y,A->sizeofmatrix)/norm(x,A->sizeofmatrix);
iterations++;

}while((rel_error > tolerance)&&(iterations < 100));//(rel_error > tolerance);

cout<<iterations<<" iterations done."<<endl;
cout<<"Error in Infinite-norm:"<<norm(x,A->sizeofmatrix)<<endl;
